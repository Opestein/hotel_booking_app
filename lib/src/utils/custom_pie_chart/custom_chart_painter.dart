...
class CustomPieChartPainter extends CustomPainter {
  ...
  CustomPieChartPainter(
    ...
    for (int i = 0; i < values.length; i++) {
      final paint = Paint()..color = getCustomColor(colorList, i);
      if (chartType == CustomChartType.ring) {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (_total == 0) {
      ...
      canvas.drawArc(
        ...
        _prevAngle,
        360,
        chartType == CustomChartType.disc ? true : false,
        paint,
      );
    } else {
  ...
  @override
  bool shouldRepaint(CustomPieChartPainter oldDelegate) =>
