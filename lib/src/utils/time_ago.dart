class TimeAgo {
  ...
  static String getTimeAgo(int timeStamp,
      {Language? language, bool isShort = false}) {
    ...
    if (difference < MINUTE_MILLIS) {
      ...
      switch (language) {
        case Language.ENGLISH:
          return "${(difference / MINUTE_MILLIS).toString().split(".")[0]} ${isShort ? 'min' : 'minutes ago'}";
        case Language.ARABIC:
      ...
      switch (language) {
        case Language.ENGLISH:
          return "${(difference / HOUR_MILLIS).toString().split(".")[0]} ${isShort ? 'hr' : 'hours ago'}";
        case Language.ARABIC:
      ...
      switch (language) {
        case Language.ENGLISH:
          return "${(difference / DAY_MILLIS).toString().split(".")[0]} ${isShort ? 'd' : 'days ago'}";
        case Language.ARABIC:
      ...
      switch (language) {
        case Language.ENGLISH:
          return (difference / MONTH_MILLIS < 2)
              ? "a month ago"
              : "${(difference / MONTH_MILLIS).toString().split(".")[0]} ${isShort ? 'm' : 'months ago'}";
        case Language.ARABIC:
          return (difference / MONTH_MILLIS < 2)
              ? "قبل شهر"
      ...
      switch (language) {
        case Language.ENGLISH:
          return (difference / YEAR_MILLIS < 2)
              ? "a year ago"
              : "${(difference / YEAR_MILLIS).toString().split(".")[0]} ${isShort ? 'y' : 'years ago'}";
        case Language.ARABIC:
          return (difference / YEAR_MILLIS < 2)
              ? "قبل عام"
